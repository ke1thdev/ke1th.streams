(() => {
  ...
  function getHomeRows(category = 'all') {
    ...
    return [
      ...
      { key: 'made_in_ph', title: 'Made in the Philippines', fetchFn: () => TMDB.discoverFilipino('popularity.desc', 1, isAll ? 'all' : category), fallbackType: catFallback, limit: 14, allowMixed: true, tileStyle: 'landscape' },
  ...
  async function switchTab(tab) {
    ...
    try {
      ...
      if (rowKey === "genre_dropdown") {
        ...
        } else if (rowKey === "made_in_ph") {
          data = await TMDB.discoverFilipino('popularity.desc', 1, targetTab);
        } else if (rowKey === "new_ph") {
